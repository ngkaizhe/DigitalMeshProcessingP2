...
vec3 ViewManager::GetWorldEyePosition() {
	...
	return getVec3(wordEyePosition);
}
...
void ViewManager::mousePressEvent(int button, int x, int y)
{
	if (button == GLUT_LEFT_BUTTON)
	{
		...
		vec4 temp = (invrtRot * vec4(0, 1, 0, 0));
		rotateYAxis = vec3(temp.x, temp.y, temp.z);
		temp = (invrtRot * vec4(1, 0, 0, 0));
		rotateXAxis = vec3(temp.x, temp.y, temp.z);
	}
...
void ViewManager::mouseMoveEvent(int x, int y)
{
	...
	else if (midDown)
	{
		...
		translationMatrix = translate(translationMatrix, (-diffUp + diffRight) * zoom * 10.0f);
		midDownCoord = coord;
	}
}
...
void ViewManager::DumpInfo(void)
{
